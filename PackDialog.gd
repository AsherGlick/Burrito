...
func _ready():
	...
	self.downloaded_markers_dir = self.user_data_dir.plus_file("marker_packs")
	self.unsaved_markers_dir = self.user_data_dir.plus_file("protobin_by_map_id")
	if not dir.dir_exists(self.downloaded_markers_dir):
		var success = dir.make_dir(self.downloaded_markers_dir)
		if success != OK:
			print("Error: Could not create data folder:", self.downloaded_markers_dir)
	if not dir.dir_exists(self.unsaved_markers_dir):
		var success = dir.make_dir(self.unsaved_markers_dir)
		if success != OK:
			print("Error: Could not create data folder:", self.unsaved_markers_dir)
